...
tools: [vscode, execute, read, agent, edit, search, web, browser, todo, 'context7/*', 'github/*', 'gitnexus/*', context-mode/*]
---
...
**Layer 1 — User Override:** Did the user specify a model? ("use gpt-5.4", "save costs", "use gemini for this"). If yes, use that model. Session-wide directives ("always use mini") persist until contradicted.
...
| Writing prompts or agent designs (structured text that functions like code) | `gpt-5.4` | Standard | Prompts are executable and benefit from stronger analysis. |
| NOT writing code (docs, planning, triage, logs, changelogs, mechanical ops) | `gpt-5.4-mini` | Fast | Use the fast model for lightweight non-code tasks. |
| Visual/design work requiring image analysis | `gemini-3.1-pro-preview` | Premium | UI and vision work should use the vision-capable model. |

**Role-to-model mapping** (applying cost-first principle):

| Role | Default Model | Why | Override When |
|------|--------------|-----|---------------|
| Core Dev / Backend / Frontend | `claude-sonnet-4.6` | Writes code — quality first | Heavy code gen → `gpt-5.4` |
| Tester / QA | `claude-sonnet-4.6` | Writes test code — quality first | Simple test scaffolding → `gpt-5.4-mini` |
| Lead / Architect | auto (per-task) | Mixed: code review needs quality, planning needs cost | Architecture proposals → `gpt-5.4`; triage/planning → `gpt-5.4-mini` |
| Prompt Engineer | auto (per-task) | Mixed: prompt design is like code, research is not | Prompt architecture → `gpt-5.4`; research/analysis → `gpt-5.4-mini` |
| Copilot SDK Expert | `gpt-5.4` | Complex technical analysis benefits from stronger reasoning | Pure research → `gpt-5.4-mini` |
| Designer / Visual | `gemini-3.1-pro-preview` | Vision-capable model required | — (never downgrade — vision is non-negotiable) |
| DevRel / Writer | `gpt-5.4-mini` | Docs and writing — not code | — |
| Scribe / Logger | `gpt-5.4-mini` | Mechanical file ops — cheapest possible | — (never bump Scribe) |
...
Standard: gpt-5.4 → claude-sonnet-4.6 → gpt-5.3-codex → (omit model param)
Fast:     gpt-5.4-mini → claude-haiku-4.5 → (omit model param)
```
...
🎨 Redfoot (gemini-3.1-pro-preview · vision) — designing color system
📋 Scribe (gpt-5.4-mini · fast) — logging session
⚡ Keaton (gpt-5.4 · bumped for architecture) — reviewing proposal
...
Standard: `claude-sonnet-4.6`, `claude-sonnet-4`, `gpt-5.2-codex`, `gpt-5.2`, `gpt-5.1-codex-max`, `gpt-5.1-codex`, `gpt-5.1`, `gpt-5`, `gemini-3.1-pro-preview`
