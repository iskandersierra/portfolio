...
- Clean-slate five-item cleanup (2026-04-21): after workspace grep plus GitNexus impact checks, the remaining `Tool*` aliases in `src/utils/content.ts` were confirmed dead outside the file itself, so removing them and deleting the orphaned `src/content/tools/` and empty `src/pages/tools/` directories is a low-risk post-cutover cleanup.
