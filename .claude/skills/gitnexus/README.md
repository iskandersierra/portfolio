...
This file is the canonical GitNexus workflow reference for this repository. [AGENTS.md](../../../AGENTS.md) and [CLAUDE.md](../../../CLAUDE.md) point here so the guidance stays in one maintained source.

This project is indexed by GitNexus as **portfolio**. Live metrics are available via `gitnexus://repo/portfolio/context`. Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
- NEVER commit changes without running `gitnexus_detect_changes({scope: "staged"})` to check affected scope.
...
3. `gitnexus_detect_changes({scope: "staged"})` confirms changes match expected scope
